...
#complicated way for learning syntax!! finds all prime numbers uptill number entered
def Sieve_Of_Eratosthenes(largestNumber):
    if largestNumber<=0:
        raise ValueError("Non negative numbers allowed, excluding zero")
    numbers = [True] * (largestNumber+1) #making list with all true values
    numbers[0] = False
    numbers[1] = False
    for i in range(2,math.ceil(largestNumber**0.5),1):
        if numbers[i] == True:
            for j in range(i**2,largestNumber+1,i): #removes numbers via i**2+ni where n is an integer
                numbers[j] = False
    primeNumbers = []
...
def Sieve_Of_Eratosthenes_Number_Of_Primes(numberOfPrimes):
    if numberOfPrimes<0:
        raise ValueError("Non negative numbers allowed, including zero")
    primeNumbers = []
    largestNumber = 2
